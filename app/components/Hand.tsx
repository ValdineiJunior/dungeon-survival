          ...
          {(() => {
            ...
            return combined.map((item, index) => {
              if (item.kind === 'default') {
                ...
                return (
                  <div key={entry.id} className="transition-all duration-200" style={{ transform: `rotate(${(index - (combined.length - 1) / 2) * 3}deg)` }}>
                    <Card
                      card={entry.card}
                      disabled={!canPlay}
                      selected={isSelected}
              ...
              return (
                <div key={card.id} className="transition-all duration-200" style={{ transform: `rotate(${(index - (combined.length - 1) / 2) * 3}deg)` }}>
                  <Card
                    card={card}
                    disabled={!canPlay}
                    selected={isSelected}
