...
export function CardListModal({ title, cards, onClose, showColors = false }: CardListModalProps) {
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/80 backdrop-blur-sm z-50">
      <div className="bg-slate-800 rounded-md md:rounded-2xl border-2 border-slate-600 shadow-2xl max-w-4xl w-full mx-4 max-h-[85vh] flex flex-col">
        {/* Header */}
        ...
        {/* Footer */}
        <div className="p-1 md:p-4 border-t border-slate-600 text-center">
          <button
            onClick={onClose}
            className="px-2 py-1 md:px-6 md:py-2 bg-slate-600 hover:bg-slate-500 text-white rounded-md md:rounded-lg transition-colors"
          >
            Fechar
          </button>
        </div>
